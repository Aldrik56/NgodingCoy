  ...
  <div class="container-fluid">
    ...
    <a data-aos="fade-right" data-aos-duration="1000" href="index.php" class="ms-5 navbar-brand logo"><img style="width: 190px; height: 50px;" src="img/logo.png"/></a>
      <ul class="navbar-nav me-auto my-lg-0 col-6 d-flex justify-content-between mx-auto" style="--bs-scroll-height: 100px;">
        <li class="nav-item">
            <a href="index.php?kategori=all" aria-current="page" class=" nav-link nav-scroll h3 text-decoration-none mt-2 text-hover" style="color:black;font-size:25px">ALL</a>
        </li>
        <li class="nav-item">
      ...
      <div class="d-flex me-5">
        <div class="profile d-flex my-auto mx-auto align-middle" style="width:100%" data-aos="fade-down" data-aos-duration="1000">
            <?php
    ...
    <div class="d-flex flex-column col-9 justify-content-center mx-auto body-section">
        <div class="mt-2 bg-img p-5 d-flex rounded" data-aos="zoom-in-left" data-aos-duration="1000"style=" height:200px;background-image:url('img/bg_profile.png');background-size: 1300px 200px;border-radius:20px;border:3px white solid;">
            <?php
        ...
        <div class="d-flex profileSection justify-content-between" >
            <div class="kiri d-block profileSection2">
                <?php
            ...
            <div class="kanan mt-3 d-inline-block profileSection2 mx-0">
                <div class="section section text-center mb-3 d-inline-block profileSectionItem2" style="width:100%;border: 1px solid;padding: 10px;box-shadow: 5px 5px #01D12F;border-radius:10px;border:1px #00B929 solid" data-aos="zoom-in-left" data-aos-duration="1000">
                    <p>Postingan</p>
                    <?php
                ...
                <div class=" section section text-center d-inline-block pt-2" style="width:100%;border: 1px solid;padding: 10px;box-shadow: 5px 5px #FF0303;border-radius:10px;border:1px #DC0000 solid;" data-aos="zoom-in-left" data-aos-duration="1000">
                    ...
                    <?php
                        if(isset($_SESSION['user_role']) && !empty($_SESSION['user_role']) && $_SESSION['user_role'] == "admin" && $row['role'] == "user") { ?>
                            <a href="ban.php?id_user=<?=$row['id']?>" class="text-body" style="text-decoration:none">⛔ Ban</a>
                    <?php
                        }
                    ?>
                    <?php
                        if(isset($_SESSION['user_role']) && !empty($_SESSION['user_role']) && $_SESSION['user_role'] == "admin" && $row['role'] == "ban") { ?>
                            <a href="unban.php?id_user=<?=$row['id']?>" class="text-body" style="text-decoration:none">⛔ Unban</a>
                    <?php
                        }
                    ?>
                    <?php
                        if(isset($_SESSION['user_role']) && !empty($_SESSION['user_role']) && $_SESSION['user_role'] == "admin" && $row['role'] == "user") { ?>
                            <a href="delete_user.php?id_user=<?=$row['id']?>" class="text-body" style="text-decoration:none">❌ Delete User</a>
                    <?php
                        }
                    ?>
                    <?php
                        if(isset($_SESSION['user_id']) && !empty($_SESSION['user_id']) && $_SESSION['user_id'] == $row['id']) { ?>
                            <a href="edit_account.php" class="text-body" style="text-decoration:none">✏️ Edit Profile</a><br>
                            <a href="logout.php" class="text-body" style="text-decoration:none">🚪➡ Log Out</a>
                    <?php
                        }
                    ?>
                </div>
    ...
    <?php
        $sqltotal = "SELECT COUNT(*) AS jumlah FROM postingan WHERE id_user = {$id_user_profile}";
        $resulttotal = $db->query($sqltotal);
        $rowtotal = $resulttotal->fetch(PDO::FETCH_ASSOC);

        if($rowtotal['jumlah'] > 1) { ?>
